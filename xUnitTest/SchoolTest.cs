...
namespace xUnitTest
{
    public class SchoolTest
    {
        ...
        public void TestGetEntity(string name, int code, DateTime createDate, string createUserId)
        {
            ...
            Assert.Equal("SELECT `Category`,`Name`,`Code`,`Adress`,`Id`,`CreateDate`,`CreateUserId`,`UpdateDate`,`UpdateUserId` FROM `School`  WHERE ((((((`Category` IN (3,0)) AND( `Name` = @Name1 )) AND ( `Code` = @Code2 )) AND ( `Category` = @Category3 )) AND ( `CreateDate` = @CreateDate4 )) AND ( `CreateUserId` = @CreateUserId5 )) ", sql.Key);
        }
